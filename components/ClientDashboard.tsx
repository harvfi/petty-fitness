          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="space-y-4">
                {foodEntries.map(f => (
                  <div key={f.id} className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 flex justify-between items-center">
                    <div>
                      <h4 className="font-bold text-white uppercase text-sm tracking-tight">{f.name}</h4>
                      <p className="text-[10px] text-zinc-500 uppercase font-black tracking-widest mt-1">{f.facts.calories} kcal • {f.facts.protein}P {f.facts.carbs}C {f.facts.fat}F</p>
                    </div>
                    <div className="text-zinc-700 font-bebas text-xl italic">{new Date(f.date).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}</div>
                  </div>
                ))}
              </div>
            </div>
